...
$completed           = 'stm-quizzes' === $current_material->post_type
	? STM_LMS_Quiz::quiz_passed( $item_id )
	: STM_LMS_Lesson::is_lesson_completed( '', $post_id, $item_id );
...
<div class="masterstudy-course-player-navigation <?php echo esc_attr( $dark_mode ? 'masterstudy-course-player-navigation_dark-mode' : '' ); ?>">
	<div class="masterstudy-course-player-navigation__wrapper">
		...
		if ( $has_access ) {
			if ( ! $completed && ! empty( $next_lesson ) ) {
			...
			} elseif ( ! $completed && empty( $next_lesson ) && ! $lesson_lock_before_start
		...
		if ( ! empty( $next_lesson ) && $completed && $has_access ) {
			?>
			<div class="masterstudy-course-player-navigation__next">
				<?php
				STM_LMS_Templates::show_lms_template(
