...
if ( ! empty( $total_progress['course']['progress_percent'] ) ) {
	$course_passed = $total_progress['course']['progress_percent'] >= STM_LMS_Options::get_option( 'certificate_threshold', 70 );
}
		...
		<div class="masterstudy-course-player-course-completed__info-success">
			<div class="masterstudy-course-player-course-completed__opportunities">
				...
				if ( ! STM_LMS_Options::get_option( 'finish_popup_image_disable', false ) ) :
					...
					$custom_failed_image  = STM_LMS_Options::get_option( 'finish_popup_image_failed' );
					$custom_success_image = STM_LMS_Options::get_option( 'finish_popup_image_success' );

					if ( ! empty( $custom_failed_image ) ) {
						$custom_failed_image_url = wp_get_attachment_image_url( $custom_failed_image );
						if ( ! empty( $custom_failed_image_url ) ) {
							$failed_image = $custom_failed_image_url;
						}
					}

					if ( ! empty( $custom_success_image ) ) {
						$custom_success_image_url = wp_get_attachment_image_url( $custom_success_image );
