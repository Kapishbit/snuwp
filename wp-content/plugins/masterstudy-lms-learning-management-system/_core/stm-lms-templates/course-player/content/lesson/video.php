	...
	if ( 'embed' === $lesson['video_type'] && ! empty( $lesson['embed_ctx'] ) ) {
		...
		if ( strpos( $uploaded_video, 'embed' ) ) {
			...
			<div class="masterstudy-course-player-lesson-video__wrapper">
				...
				<video id="masterstudy-course-player-lesson-video" data-id="<?php echo esc_attr( $id ); ?>" poster="<?php echo esc_url( $lesson['video_poster']['url'] ?? '' ); ?>" controls
					controlsList="nodownload" style="<?php echo esc_attr( ! empty( $video_width ) ? $video_width : '' ); ?>">
					<source src="<?php echo esc_url( $uploaded_video ); ?>" type='video/<?php echo esc_attr( $video_format ); ?>'>
				</video>
			</div>
			<?php
		}
		...
		?>
		<iframe src="<?php // phpcs:ignore Squiz.PHP.EmbeddedPhp
						echo esc_attr(
