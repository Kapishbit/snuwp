...
<div class="masterstudy-course-player-quiz <?php echo esc_attr( $data['show_answers'] ? 'masterstudy-course-player-quiz_show-answers' : '' ); ?>">
	...
	if ( ! empty( $data['last_quiz'] ) ) {
		...
		STM_LMS_Templates::show_lms_template(
			...
			array(
				...
				'passing_grade'      => ! empty( $data['passing_grade'] ) ? intval( $data['passing_grade'] ) : 0,
				'questions_quantity' => ! empty( $data['questions_quantity'] ) ? intval( $data['questions_quantity'] ) : 0,
	...
	if ( ! empty( $data['quiz_meta']['questions'] ) ) {
		...
		<form class="masterstudy-course-player-quiz__form <?php echo esc_attr( ! $data['show_answers'] || empty( $data['last_quiz'] ) ? 'masterstudy-course-player-quiz__form_hide' : '' ); ?>">
			...
			if ( ! $data['passed'] ) {
				?>
				<input type="hidden" name="question_ids" value="<?php echo esc_attr( is_array( $data['questions_ids'] ) ? implode( ',', $data['questions_ids'] ) : '' ); ?>"/>
