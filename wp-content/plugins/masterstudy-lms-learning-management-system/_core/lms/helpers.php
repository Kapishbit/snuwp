...
function ms_plugin_curriculum_data( $course_id, $materials ) {
	$lesson_types_labels = array(
		...
		'stm-quizzes'      => __( 'Quiz', 'masterstudy-lms-learning-management-system' ),
		'stm-assignments'  => __( 'Assignment', 'masterstudy-lms-learning-management-system' ),
		...
		'stm-google-meets' => __( 'Google Meet webinar', 'masterstudy-lms-learning-management-system' ),
	);
	if ( ! empty( $course_id ) && ! empty( $materials ) ) {
		return array_map(
			function( $material ) use ( $course_id, $lesson_types_labels ) {
				$material['post_id']                  = stm_lms_get_wpml_binded_id( $material['post_id'] );
				$material['lesson_type']              = ! empty( $material['lesson_type'] ) ? $material['lesson_type'] : 'text';
				$material['lesson_excerpt']           = get_post_meta( $material['post_id'], 'lesson_excerpt', true );
				$material['has_access']               = STM_LMS_User::has_course_access( $course_id, $material['post_id'] );
				$material['lesson_lock_before_start'] = false;
				$material['lesson_locked_by_drip']    = false;
				if ( 'stm-quizzes' === $material['post_type'] ) {
					$material['completed']       = STM_LMS_Quiz::quiz_passed( $material['post_id'] ) ? 'completed' : '';
					$material['icon']            = 'quiz';
					...
					$material['label']           = $lesson_types_labels[ $material['post_type'] ];
				} else {
					$material['completed'] = STM_LMS_Lesson::is_lesson_completed( '', $course_id, $material['post_id'] ) ? 'completed' : '';
					$material['icon']      = ! empty( $material['lesson_type'] ) ? $material['lesson_type'] : 'text';
					$material['meta']      = '';
					...
					if ( 'stm-assignments' === $material['post_type'] ) {
						...
						$material['meta']  = '';
						$material['label'] = $lesson_types_labels[ $material['post_type'] ];
					} elseif ( 'stm-google-meets' === $material['post_type'] ) {
...
function ms_plugin_curriculum_trial_data( $course_id ) {
	$is_trial_course = get_post_meta( $course_id, 'shareware', true );
	$trial_access    = false;
	if ( ! empty( $is_trial_course ) && 'on' === $is_trial_course && class_exists( 'STM_LMS_Shareware' ) ) {
		...
		$trial_lesson_count = array_filter(
			$course_materials,
			function ( $lesson ) use ( $trial_lessons ) {
				return ( $trial_lessons >= $lesson['order'] );
			}
		);
		if ( ! empty( $trial_lesson_count ) ) {
			$trial_access = true;
		}
	}
...
function ms_plugin_base_colors() {
	...
	$generated_colors = ms_plugin_generate_base_colors( $custom_colors );
	?>
	<style>
		:root {
			<?php foreach ( $generated_colors as $name => $colors ) { ?>
				--<?php echo esc_html( $name ); ?>-100: <?php echo esc_html( $colors['100'] ); ?>;
				--<?php echo esc_html( $name ); ?>-70: <?php echo esc_html( $colors['70'] ); ?>;
				--<?php echo esc_html( $name ); ?>-50: <?php echo esc_html( $colors['50'] ); ?>;
				--<?php echo esc_html( $name ); ?>-30: <?php echo esc_html( $colors['30'] ); ?>;
				--<?php echo esc_html( $name ); ?>-10: <?php echo esc_html( $colors['10'] ); ?>;
				--<?php echo esc_html( $name ); ?>-5: <?php echo esc_html( $colors['5'] ); ?>;
				--<?php echo esc_html( $name ); ?>-hover: <?php echo esc_html( $colors['hover'] ); ?>;
			<?php } ?>
		}
	</style>
	<?php
...
function ms_plugin_generate_base_colors( $base_colors ) {
	$generated_colors = array();

	foreach ( $base_colors as $name => $color ) {
		sscanf( $color, 'rgba(%d, %d, %d, %f)', $first, $second, $third, $opacity );

		$generated_colors[ $name ] = array(
			'100'   => $color,
			'70'    => "rgba($first, $second, $third, 0.7)",
			'50'    => "rgba($first, $second, $third, 0.5)",
			'30'    => "rgba($first, $second, $third, 0.3)",
			'10'    => "rgba($first, $second, $third, 0.1)",
			'5'     => "rgba($first, $second, $third, 0.05)",
			'hover' => "rgba($first, $second, $third, 0.85)",
		);
	}
