		...
		foreach ( $_POST as $question_id => $value ) {
			if ( is_numeric( $question_id ) ) {
				...
				if ( 'fill_the_gap' === $type ) {
					$answer = self::encode_answers( self::deslash( $value ) );
				} else {
					if ( is_array( $value ) ) {
		...
		foreach ( $answers as $stored_answer ) {
			switch ( $type ) {
				...
				case 'multi_choice':
					...
					$answer_contains_full_answer  = in_array( $full_answer, $answer, true ) && $stored_answer['isTrue'];
					$answer_contains_wrong_answer = in_array( $full_answer, $answer, true ) && ! $stored_answer['isTrue'];

					if ( $answer_contains_full_answer ) {
						$correct = true;
					} elseif ( $answer_contains_wrong_answer ) {
						$correct          = false;
