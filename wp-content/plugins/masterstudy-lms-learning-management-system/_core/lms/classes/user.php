...
class STM_LMS_User {
	...
	public static function stm_lms_login() {
		...
		$get_user_by   = is_email( $data['user_login'] ) ? 'email' : 'login';
		$is_registered = get_user_by( $get_user_by, $data['user_login'] );
		if ( ! $is_registered ) {
			$response['errors'][] = array(
				'id'    => 'wrong_username',
				'field' => 'user_login',
	...
	public static function stm_lms_register() {
		...
		/*Add check for login*/
		if ( sanitize_user( $data['register_user_login'], true ) !== $data['register_user_login'] ) {
			$response['errors'][] = array(
				'id'    => 'tags',
	...
	public static function has_course_access( $course_id, $item_id = '', $add = true ) {
		...
		if ( ! count( $course ) ) {
			...
			if ( $is_free && $prerequisite_passed && $add ) {
				...
				if ( $auto_enroll || ( ! $auto_enroll && ! is_single() ) ) {
					STM_LMS_Course::add_user_course( $course_id, $user_id, STM_LMS_Course::item_url( $course_id, '' ), 0 );
				}
